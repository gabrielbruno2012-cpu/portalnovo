...
// CORS completo (para Base44 preview, PWA, mobile, etc.)
app.use(cors({
  origin: '*',
...
// ======================================
// COLABORADORES (somente role=colaborador)
// ======================================
...
app.get('/api/producao/colaborador', (req, res) => {
    ...
    const sql = `
        SELECT 
            ...
            p.nota,
            p.nota_status,
            ed.parcela_numero AS emprestimo_parcela_numero,
            ed.total_parcelas AS emprestimo_total_parcelas,
            ed.valor_parcela AS emprestimo_valor_parcela
        FROM producao_colaborador p
        LEFT JOIN usuarios u ON u.id = p.usuario_id
        LEFT JOIN emprestimo_descontos ed ON ed.producao_id = p.id
        WHERE p.usuario_id = ?
